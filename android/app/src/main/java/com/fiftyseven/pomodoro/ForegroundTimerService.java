...
public class ForegroundTimerService extends Service {
    ...
    private final Runnable mTick = new Runnable() {
        @Override public void run() {
            // 倒计时往下降；负数表示加时正计时，往上涨
            if (!sPaused) {
                if (sRemainSec < 0) sRemainSec++;
                else if (sRemainSec > 0) sRemainSec--;
            }
            updateNotification();
    ...
    private Notification buildNotification() {
        // 负数秒 = 加时正计时（专注倒计时走完后继续数的那段）
        boolean overtime = sRemainSec < 0;
        String phase = overtime ? "加时专注" : (sIsFocus ? "专注中" : "休息中");
        String when = sPaused ? "已暂停 · " : "";
        int sec = Math.abs(sRemainSec);
        String time = (overtime ? "+" : "") + String.format("%02d:%02d", sec / 60, sec % 60);
        ...
        return new NotificationCompat.Builder(this, CHANNEL_FGS)
                ...
                .setContentTitle(when + phase + " " + time)
                .setContentText(overtime ? "超出时间也在计入专注"
                                  : (sPaused ? "已暂停，点继续接着专注" : "保持专注，点开可查看详情"))
                .setOngoing(true)
