...
def main():
    icon = load("icon.png")
    # 小密度用简化版（数字更粗、无底卡、细节更少），48/72px 才不糊
    icon_small = load("icon-small.png")
    fg = load("icon-foreground.png")
    fg_small = load("icon-foreground-small.png")
    ...
    for d, px in ICON_DP.items():
        ...
        # 96px 以下用简化版源图
        src = icon_small if px <= 96 else icon
        ...
        fpx = FG_DP[d]
        fsrc = fg_small if fpx <= 216 else fg
        fsrc.resize((fpx, fpx), Image.LANCZOS).save(
