...
function makeApp(opts = {}) {
  // 页面里的未捕获异常（含定时器回调）默认会被 jsdom 吞掉，接出来方便定位
  const vc = new VirtualConsole();
  vc.on('jsdomError', (e) => {
    console.log('  [jsdomError]', String((e && e.message) || e).slice(0, 160));
  });
...
console.log('\n[F] 完成专注 → 统计 + 任务 +1');
{
  ...
  advance(app, 61_000);
  await flush();
  app.$('startBtn').click();   // 加时态按暂停 = 结算这次专注
  await flush(); await flush();
  ...
  check('结算后处于暂停态', app.w.PomodoroTimer.get().paused === true);
  app.$('skipBtn').click();    // 结束加时，进入短休息
  await flush();
  check('跳过加时切到短休息', app.w.PomodoroTimer.get().phase === 'short');
...
console.log('\n[AB] 中断记录：暂停计入 session');
{
  ...
  app.$('startBtn').click();
  await flush();
  advance(app, 41_000);               // 跑完剩余，进入加时
  await flush();
  app.$('startBtn').click();          // 加时态暂停 = 结算（两次中断一并入账）
  await flush(); await flush();
...
console.log('\n[AH] 加时态 UI：+MM:SS 显示、提醒、翻页钟切换');{
  ...
  check('表盘显示 +00:01', app.$('timeDisplay').textContent === '+00:01',
        app.$('timeDisplay').textContent);
  check('阶段标签为加时专注', app.$('phaseLabel').textContent === '加时专注');
  check('按钮仍为暂停', app.$('startBtn').textContent === '暂停');
  check('加时开始有提示音', app.audioCalls.length >= 3, app.audioCalls.length);
  check('加时开始有震动', app.calls.notification.length >= 1);
  check('到点后撤掉备用闹钟', app.calls.cancel.length >= 1, app.calls.cancel.length);

  // 翻页钟：手动切换 当前时间 / 倒计时（加时态默认显示加时正计时）
  app.$('flipBtn').click();
  await flush(); await flush();
  check('加时态默认显示倒计时', app.$('flipSub').textContent === '加时',
        app.$('flipSub').textContent);
  app.$('flipMode').click();
  await flush();
  check('切到时钟模式', app.$('flipSub').textContent === '' && /\d{2}:\d{2}/.test(
    app.$('flipStack').textContent), app.$('flipStack').textContent);
  app.$('flipMode').click();
  await flush();
  check('切回倒计时显示 +00:XX', app.$('flipSub').textContent === '加时',
        app.$('flipSub').textContent);
  app.$('flipExit').click();
  await flush();

  // 暂停结算：超出的 1 秒不足以凑满 1 分钟，分钟数仍是 1
  app.$('startBtn').click();
  await flush(); await flush();
  check('结算后表盘保持 +MM:SS', app.$('timeDisplay').textContent.startsWith('+'),
        app.$('timeDisplay').textContent);
  check('结算只记一条', app.w.PomodoroStore.loadSessions().length === 1);
  check('提示文案不报错', app.$('hintLabel').textContent.length > 0);
}
...
console.log('\n[AI] 半年热力图 + 自动连跑设置');
{
  ...
  app.$('panelBtn').click();
  await flush();
  app.$('chartTabs').querySelector('[data-chart="year"]').click();
  await flush();
  check('半年热力图已渲染', !!app.$('mainChart').querySelector('.year-grid'),
        app.$('mainChart').innerHTML.slice(0, 60));
  check('热力图 20 列 × 7 行',
        app.$('mainChart').querySelectorAll('.y-cell').length === 140);
  check('范围切换对热力图隐藏', app.$('rangeTabs').hidden === true);
  app.$('chartTabs').querySelector('[data-chart="trend"]').click();
  await flush();
  check('切回趋势后范围条恢复', app.$('rangeTabs').hidden === false);

  // 设置开关回显与生效 + 刷新后保留
  const st = new Map();
  const app2 = makeApp({ storage: st });
  await flush();
  check('自动休息默认关', app2.$('autoBreakToggle').checked === false);
  check('自动下一轮默认关', app2.$('autoFocusToggle').checked === false);
  app2.$('autoBreakToggle').click();
  await flush();
  app2.$('autoFocusToggle').click();
  await flush();
  check('开关写入设置',
        app2.w.PomodoroTimer.getSettings().autoStartBreak === true &&
        app2.w.PomodoroTimer.getSettings().autoStartFocus === true);
  const app3 = makeApp({ storage: st });
  await flush();
  check('刷新后开关保留',
        app3.$('autoBreakToggle').checked === true &&
        app3.$('autoFocusToggle').checked === true);
}
