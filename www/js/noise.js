 ...
 *   wind  — 棕噪带通，双 LFO 慢扫中心频率与响度，山谷风一阵一阵
 *   fire  — 棕噪低通做火堆底 + 随机短促爆裂粒（篝火的噼啪）
 *
...
(function (global) {
  ...
  function build(kind, c, out) {
    ...
    if (kind === 'wind') {
      var wm = c.createGain(); wm.gain.value = 0.18;

      // 山谷风：带通中心频率被两个慢 LFO 推着走，一阵紧一阵松
      var bp = c.createBiquadFilter();
      bp.type = 'bandpass'; bp.Q.value = 0.9; bp.frequency.value = 320;
      var ws = src(c, B('brown')); ws.connect(bp); bp.connect(wm); sources.push(ws);

      sources.push.apply(sources, lfo(c, 0.045, 190, bp.frequency));
      sources.push.apply(sources, lfo(c, 0.023, 120, bp.frequency));
      sources.push.apply(sources, lfo(c, 0.06, 0.07, wm.gain));

      wm.connect(out);
      return { master: wm, sources: sources };
    }

    if (kind === 'fire') {
      var fm = c.createGain(); fm.gain.value = 0.22;

      // 火堆底：棕噪低通，稳稳的暖
      var lp = c.createBiquadFilter();
      lp.type = 'lowpass'; lp.frequency.value = 620;
      var fs = src(c, B('brown')); fs.connect(lp); lp.connect(fm); sources.push(fs);
      sources.push.apply(sources, lfo(c, 0.09, 0.05, fm.gain));

      // 噼啪：随机时刻放一粒极短的爆裂噪声包络
      var grain = c.createBuffer(1, Math.floor(c.sampleRate * 0.04), c.sampleRate);
      var gd = grain.getChannelData(0);
      for (var i = 0; i < gd.length; i++) {
        gd[i] = (Math.random() * 2 - 1) * (1 - i / gd.length);
      }
      var crackle = c.createGain(); crackle.gain.value = 0.5;
      crackle.connect(fm);
      sources.push(crackle);
      (function pop() {
        if (playing && playing.kind === 'fire') {
          var g = c.createBufferSource();
          g.buffer = grain;
          try { g.playbackRate.value = 0.7 + Math.random() * 0.9; } catch (e) {}
          g.connect(crackle);
          g.start();
          sources.push(g);
        }
        global.setTimeout(pop, 120 + Math.random() * 900);
      })();

      fm.connect(out);
      return { master: fm, sources: sources };
    }
