...
(function (global) {
  ...
  function renderMain(s) {
    ...
    el.autoBreakToggle.checked = s.settings.autoStartBreak;
    el.autoFocusToggle.checked = s.settings.autoStartFocus;
  ...
  function renderMainChart() {
    ...
    // 半年热力图是固定 20 周窗口，范围切换对它无意义
    el.rangeTabs.hidden = (currentChart === 'year');
    ...
    if (currentChart === 'trend') {
      ...
      box.innerHTML = Charts.yearGrid(Store.yearHeatmap(20));
      note.textContent = '过去 20 周：颜色越深，当天完成得越多';
    } else {
  ...
  function onPhaseEnd(info) {
    ...
    // 提示音分工：加时开始已响过"专注铃"；自动连跑的自然完成不走加时，
    // 在这里补响；结算时用户刚按过按钮（有按压震动），不叠加铃声
    if (info.counted && wasFocus && !info.finalizedBy) {
  ...
  function bindSettings() {
    ...
    el.noiseSelect.addEventListener('change', function () {
      ...
      toast(({ rain: '雨声', cafe: '咖啡馆', wind: '山风', fire: '篝火' })[this.value] || '已关闭白噪音', 1400);
    });
    ...
    // 自动连跑：休息自动开始 / 下一轮自动开始
    el.autoBreakToggle.addEventListener('change', function () {
      T.setSetting('autoStartBreak', this.checked);
      toast(this.checked ? '专注到点将自动开始休息' : '到点后进入加时，由你决定何时休息', 2400);
    });
    el.autoFocusToggle.addEventListener('change', function () {
      T.setSetting('autoStartFocus', this.checked);
      toast(this.checked ? '休息结束将自动开始下一轮' : '休息结束等你手动开始', 2000);
    });
  ...
  function boot() {
    [
      ...
      'noiseSelect', 'fgsToggle', 'autoBreakToggle', 'autoFocusToggle',
      'dailyGoal', 'flipBtn', 'toast'
