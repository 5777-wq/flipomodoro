...
(function (global) {
  ...
  function renderMain(s) {
    ...
    if (s.running) {
      el.hintLabel.textContent = s.isFocus ? '保持专注' : '放松一下';
    } else if (s.elapsedMs > 0) {
      el.hintLabel.textContent = '已暂停';
    } else {
    ...
    el.todayCount.textContent = String(rec.count);
  ...
  function renderStats() {
    ...
    el.statToday.textContent = String(rec.count);
    el.statMinutes.textContent = String(rec.minutes);
    el.statStreak.textContent = String(ins.streak);
  ...
  function onPhaseEnd(info) {
    ...
    if (info.counted && wasFocus) {
      // 只有走完全程才写记录；interrupts 是这次专注里的暂停次数
      var t = activeTask();
    ...
    if (s.settings.sound) {
      wasFocus ? Sound.focusEnd() : Sound.restEnd();
    }
    if (s.settings.vibrate) Native.notificationVibrate();

    // App 在后台时（FGS 让进程活着、JS 只是被节流不冻结），
    // 阶段结束要补一条即时通知——用户不在 app 里，光有声音不够。
    // 此刻备用闹钟已被 cancelEndNotifications 撤销，这里补"给人看"的那条。
    // 用独立 id（8803/8804），与备用闹钟的 8801/8802 互不干扰。
    if (global.document && document.hidden && s.settings.notify) {
      Native.postImmediate(
        wasFocus ? 8803 : 8804,
        wasFocus ? '专注结束' : '休息结束',
        wasFocus ? '休息一下吧' : '回到专注，继续下一个番茄'
      );
    }
  ...
  function bindControls() {
    el.startBtn.addEventListener('click', function () {
      ...
      // 专注进行中按下 = 暂停，记一次中断
      if (!willStart && snap.running && snap.isFocus) focusPauses++;
  ...
  function initNative() {
    ...
    Native.onFgsAction(function (act) {
      if (act === 'toggle') {
        ...
        if (s.running && s.isFocus) focusPauses++;
        T.toggle();
        if (T.getSettings().vibrate) Native.impact('LIGHT');
      } else if (act === 'stop') {
        Noise.stop();
        ...
        // 先 resync：锁屏期间阶段可能已经自然完成，直接 start 会把
        // 旧阶段跑起来，随后完成结算又把它停掉，表现为"点了没反应"。
        T.resync();
        cancelEndNotifications();
        Sound.unlock();
        T.start();
