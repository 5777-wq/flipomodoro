...
(function (global) {
  ...
  var DEFAULT_SETTINGS = {
    ...
    autoStartBreak: false, // 专注到点自动开始休息（开启后不进加时，走严格番茄流程）
    autoStartFocus: false  // 休息结束自动开始下一轮专注
  };
  ...
  var NOISE_KINDS = { off: 1, rain: 1, cafe: 1, wind: 1, fire: 1 };
  ...
  var Store = {
    ...
    /**
     * 半年热力图（GitHub 贡献图风格）：按周分列、每周 7 行。
     * @param {number} weeks 周数，默认 20
     * @returns {{cols: Array<{days: Array<{day, count, level, future}>}>, max}}
     *   level 0..4 是颜色档位，future=true 表示还没到的日子
     */
    yearHeatmap: function (weeks) {
      weeks = clamp(weeks || 20, 4, 26);
      var recs = Store.loadRecords();
      var today = new Date();
      var todayKey = dayKey(today);
      // 从本周的周一起往回排，最后一列的末尾就是这周
      var offMon = (today.getDay() + 6) % 7;
      var start = shiftDays(today, -offMon - (weeks - 1) * 7);
      ...
      for (var w = 0; w < weeks; w++) {
        var days = [];
        for (var i = 0; i < 7; i++) {
          var day = shiftDays(start, w * 7 + i);
          var k = dayKey(day);
          var count = recs[k] ? recs[k].count : 0;
          if (count > max) max = count;
          days.push({ day: k, count: count, future: k > todayKey });
        }
        cols.push({ days: days });
      }
      // 颜色档位 1..4：按相对最大值的占比分档（0 = 当天没做）
      cols.forEach(function (col) {
        col.days.forEach(function (d) {
          d.level = d.count > 0
            ? Math.max(1, Math.min(4, Math.ceil(d.count / max * 4)))
            : 0;
        });
      });
