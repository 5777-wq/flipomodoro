...
(function (global) {
  ...
  // 用户手动选定的模式（'clock' | 'timer' | null=自动）。
  // 自动规则：计时中显示倒计时，空闲显示当前时间；手动切换后以用户为准。
  var manualMode = null;
  ...
  function renderTimer(snap) {
    var txt, label;
    if (snap.overtime) {
      // 加时正计时：卡片显示超出的 MM:SS，"加时"二字标在下方
      txt = fmtRemain(Math.ceil(snap.overtimeMs / 1000));
      label = '加时';
    } else {
      txt = fmtRemain(snap.remainSec);
      label = snap.isFocus ? '专注' : (snap.phase === 'longBreak' ? '长休息' : '短休息');
    }
  ...
  var api = {
    ...
    enter: function () {
      ...
      // 首帧按当前模式画（手动选择 > 计时中倒计时 > 空闲时钟）
      api.feed(global.PomodoroTimer.get());
      showChrome();
      el.flipExit.focus();
    ...
    /**
     * 主循环喂帧：计时中的每一拍调它。
     * 模式优先级：用户手动选择 > 自动规则（计时中→倒计时，空闲→时钟）。
     */
    feed: function (snap) {
      if (!open) return;
      var mode = manualMode || ((snap.running || snap.elapsedMs > 0) ? 'timer' : 'clock');
      if (mode === 'timer') renderTimer(snap);
      else renderClock();
    },
    ...
    bind: function () {
      ...
      root = el.flipclock;
      ['flipStack', 'flipSub', 'flipDate', 'flipExit', 'flipMode'].forEach(function (id) {
      ...
      // 手动切换 当前时间 / 倒计时（加时时是加时正计时）
      if (el.flipMode) {
        el.flipMode.addEventListener('click', function (e) {
          e.stopPropagation();
          manualMode = (manualMode === 'clock') ? 'timer' : 'clock';
          api.feed(global.PomodoroTimer.get());
          showChrome();
          if (global.PomodoroNative) global.PomodoroNative.impact('LIGHT');
        });
      }
    }
  };
