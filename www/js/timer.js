...
(function (global) {
  ...
  var focusOverride = 0;
  // 加时态：专注倒计时走完后不停车，转成正计时。
  // 超出的时间同样计入专注时长，直到用户暂停（结算）或开始休息（结算）。
  var overtime = false;
  // 本阶段是否已写入完成记录（防止"暂停结算→继续→再结算"重复记账）
  var recorded = false;
  var tickHandle = null;
  ...
  var listeners = { tick: [], phaseEnd: [], change: [], overtimeStart: [] };
  ...
  /** 加时分钟数：正计时超出设定时长的部分，按分钟取整 */
  function overtimeMin() {
    if (!overtime) return 0;
    var ot = elapsedMs() - phaseDurationMs(phase);
    return Math.max(0, Math.round(ot / 60000));
  }

  /** 专注阶段结束后（含加时）下一阶段该是什么 */
  function nextPhaseAfterFocus() {
    return (doneInRound >= settings.roundsBeforeLong) ? PHASE.LONG : PHASE.SHORT;
  }
  ...
  function restore() {
    ...
    overtime = !!(s.overtime && phase === PHASE.FOCUS);
    recorded = !!s.recorded;
    ...
    // 加时态不把已跑时间夹紧到设定时长——超出部分也是专注时间。
    // 封顶 12 小时，防脏数据把表盘撑爆。
    if (overtime) {
      elapsedBefore = Math.min(before, phaseDurationMs(phase) + 12 * 3600 * 1000);
    } else {
  ...
  /**
   * 推进到下一阶段。
   * @param {boolean} count 是否计入完成记录（自然跑完/加时结算计，手动跳过不计）
   *
   * 注意 doneInRound：加时开始（enterOvertime）时已经 +1 过了，
   * 所以从加时态推进时不能重复计数——这里直接读运行时 overtime 标志判断。
   */
  function advance(count) {
    var finished = phase;
    var wasOvertime = overtime;
    // 记录口径要真实：该番茄实际跑了多久（可能被任务自定义时长覆盖，
    // 加时结算时还要把正计时的部分算进去），而不是设置里的全局值。
    var focusMin = (finished === PHASE.FOCUS)
      ? (wasOvertime ? effFocusMin() + overtimeMin() : effFocusMin())
      : settings.focusMin;

    if (finished === PHASE.FOCUS) {
      if (!wasOvertime) doneInRound++;
      phase = nextPhaseAfterFocus();
    } else {
      // 长休息结束代表新一轮开始
  ...
  /**
   * 专注倒计时走完：响铃提醒但【不停车】，转成加时正计时。
   * doneInRound 在这里 +1，后面从加时结算/跳过时不再重复。
   */
  function enterOvertime() {
    overtime = true;
    recorded = false;
    if (phase === PHASE.FOCUS) doneInRound++;
    lastEmittedSec = -1;
    persist();
    emit('overtimeStart', snapshot());
    emit('change', snapshot());
  }

  /**
   * 结算加时（用户在加时态按了暂停，或开始休息）。
   * 只发一次：recorded 标志防止"暂停结算→继续→再结算"重复记账。
   * @returns {boolean} 是否真的结算了
   */
  function maybeFinalizeOvertime(reason) {
    if (!overtime || phase !== PHASE.FOCUS || recorded) return false;
    recorded = true;
    emit('phaseEnd', {
      finished: PHASE.FOCUS,
      next: nextPhaseAfterFocus(),
      counted: true,
      focusMin: effFocusMin() + overtimeMin(),
      overtime: true,
      finalizedBy: reason
    });
    return true;
  ...
  function onTick() {
    ...
    if (remainingMs() <= 0) {
      // 严格番茄模式：专注到点直接自动开始休息（不进加时）
      if (phase === PHASE.FOCUS && settings.autoStartBreak) {
        completeCurrentPhase();
        API.start();
        return;
      }
      if (phase === PHASE.FOCUS) {
        // 倒计时走完不停车：转加时正计时，继续往下走；
        if (!overtime) enterOvertime();
      } else {
        // 休息走完停下车；开了"下一轮自动开始"就连着跑
        completeCurrentPhase();
        if (settings.autoStartFocus) API.start();
        return;
      }
    }
    ...
    // 加时态的"秒"是正计时秒数，同样每秒播报一次
    var sec = overtime
      ? Math.floor((elapsedMs() - phaseDurationMs(phase)) / 1000)
  ...
  function completeCurrentPhase() {
    ...
    // 休息走完自动进入下一轮专注（设置开启时）
    if (!wasFocus && settings.autoStartFocus) API.start();
  }
  ...
  function snapshot() {
    ...
    return {
      ...
      /** 加时正计时：专注倒计时走完后为 true */
      overtime: overtime && phase === PHASE.FOCUS && elapsedMs() >= phaseDurationMs(phase),
      /** 加时毫秒数（非加时态为 0） */
      overtimeMs: overtime
        ? Math.max(0, elapsedMs() - phaseDurationMs(phase))
        : 0,
  ...
  var API = {
    ...
    start: function () {
      if (isActive()) return;
      // 已跑完则从头开始（加时态例外：续跑要接着正计时，不能清零）
      if (remainingMs() <= 0 && !overtime) elapsedBefore = 0;
      startAt = Date.now();
    ...
    pause: function () {
      if (!isActive()) return;
      // 加时态按暂停 = 结算这次专注（响铃/通知已在加时开始时发过）
      maybeFinalizeOvertime('pause');
      // 把已跑的这一段固化进 elapsedBefore，总时长不变。
      // 加时态不夹紧到设定时长——超出的部分也是专注时间。
      elapsedBefore = overtime
        ? elapsedMs()
        : Math.min(elapsedMs(), phaseDurationMs(phase));
      startAt = 0;
      state = STATE.PAUSED;
      stopTick();
      persist();
    ...
    /**
     * 手动跳过：推进阶段。
     * 倒计时内的跳过不计入完成；加时态的跳过 = 结算这次专注（计入）；
     * 已经按暂停结算过的话就不再重复计。
     */
    skip: function () {
      var wasActive = isActive();
      var wasOvertime = overtime && phase === PHASE.FOCUS && !recorded;
      stopTick();
      var info = advance(wasOvertime);
      state = STATE.IDLE;
      persist();
      emit('phaseEnd', info);
      emit('change', snapshot());
