...
(function (global) {
  ...
  var Charts = {
    ...
    trendLine: function (series, goal) {
      ...
      // 折线
      svg.push('<polyline class="trend-draw" points="' + linePts.join(' ') + '" fill="none" stroke="' + ORANGE
    ...
    weekdayBars: function (rows) {
      ...
      rows.forEach(function (r, i) {
        ...
        svg.push('<rect class="wk-rect" x="' + x + '" y="' + y + '" width="' + barW + '" height="' + num(h)
    ...
    /**
     * 半年热力图（GitHub 贡献图风格）：列 = 周，行 = 周一~周日。
     * @param {{cols:Array}} data Store.yearHeatmap() 的结果
     */
    yearGrid: function (data) {
      if (!data || !data.cols || !data.cols.length) {
        return '<p class="empty-tip">还没有数据</p>';
      }
      var cells = [];
      data.cols.forEach(function (col) {
        col.days.forEach(function (d) {
          var cls = 'y-cell' + (d.future ? ' y-future' : '');
          cells.push('<i class="' + cls + '" data-l="' + d.level + '" title="'
                   + esc(d.day) + ' · ' + d.count + ' 个"></i>');
        });
      });
      var first = data.cols[0].days[0].day.split('-');
      var lastCol = data.cols[data.cols.length - 1].days;
      var last = lastCol[lastCol.length - 1].day.split('-');
      return '<div class="year-grid" role="img" aria-label="过去 '
           + data.cols.length + ' 周的专注热力图">' + cells.join('') + '</div>'
           + '<div class="chart-axis"><span>' + esc(first[0]) + '年' + esc(first[1])
           + '月</span><span>' + esc(last[0]) + '年' + esc(last[1]) + '月</span></div>';
    },
